...
if(t==='slider'){const notch=(p===P.hpfFreq&&paramValues[P.adsrMode]>=0.5)?4:0;drawSlider(x,y,paramValues[p],e||0,notch)}
...
else if(t==='btnled'){pressedBtn=ci;paramValues[p]=paramValues[p]>0.5?0:1;synth.setParam(p,paramValues[p]);
if(p===P.hold&&paramValues[p]<=0.5)kbKeys.fill(0);
if(p===P.transpose&&paramValues[p]<=0.5){transposeKey=-1;synth.setParam(P.transposeOfs,0)}
drawAll()}
else if(t==='power'){paramValues[p]=paramValues[p]>0.5?0:1;synth.setParam(p,paramValues[p]);drawAll()}
else if(t==='bender'){dragCtrl=ci;dragStartY=e.clientX;dragStartVal=paramValues[p]}
else if(t==='chorusoff'){pressedBtn=ci;paramValues[P.chorusI]=0;paramValues[P.chorusII]=0;synth.setParam(P.chorusI,0);synth.setParam(P.chorusII,0);drawAll()}
e.preventDefault()});
...
function showTip(ci){
  ...
  const isVal=t==='slider'||t==='knob';
  const val=isVal?paramValueText(p,paramValues[p]):'';
  if(!val){tt.style.display='none';tipCtrl=-1;return}
  tt.textContent=val;
...
if(scopeMode===3&&pos.x>=SC.x&&pos.x<SC.x+SC.w&&pos.y>=SC.y&&pos.y<SC.y+SC.h){
...
if(t==='slider'||t==='knob'){
  ...
  let nv=dragAccum;
  if(p===P.hpfFreq&&paramValues[P.adsrMode]>=0.5)nv=Math.round(nv*3)/3;
  paramValues[p]=nv;synth.setParam(p,p===P.hpfFreq?nv*3:nv)}
else if(t==='switch3'||t==='switch2'){const mx=t==='switch3'?2:1;paramValues[p]=Math.max(0,Math.min(mx,Math.round(dragStartVal+(e.clientY-dragStartY)/12)));synth.setParam(p,paramValues[p])}
else if(t==='hswitch3'||t==='hswitch2'){const mx=t==='hswitch3'?2:1;paramValues[p]=Math.max(0,Math.min(mx,Math.round(dragStartVal+(e.clientX-dragStartY)/12)));synth.setParam(p,paramValues[p]);
if(p===P.adsrMode){synth.setBankOffset(paramValues[p]<0.5?0:1);synth.loadPreset(currentPreset);syncFP(currentPreset)}}
...
if(dragCtrl!==null){
  ...
  if(dt==='bender'){paramValues[P.bender]=0.5;synth.setParam(P.bender,0);drawAll()}
...
function drawPresetSheet(){
  ...
  for(let i=0;i<128;i++){
    ...
    const name=synth.getPresetName(i).substring(0,14);
...
try{
  ...
  drawAll();
  await synth.init();syncFP(0);
  paramValues[P.tuning]=0.5;paramValues[P.masterVol]=0.5;paramValues[P.bender]=0.5;
  paramValues[P.power]=1;paramValues[P.arpRate]=0.35;
  drawAll();
  try{await synth.initMidi()}catch(e){}
  let lastFrame=0;function scopeLoop(t){requestAnimationFrame(scopeLoop);if(t-lastFrame<33)return;lastFrame=t;
