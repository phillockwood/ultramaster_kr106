...
let scopeMode=0;
const SCOPE_MODES=6; // 0=wave, 1=spectrum, 2=adsr, 3=patchbank, 4=vcf, 5=about
...
let newSamples=(writePos-scopeLocalReadPos+ringSize)%ringSize;
if(newSamples===0)return;
let peak=0;
for(let i=0;i<newSamples;i++){
  const srcIdx=(scopeLocalReadPos+i)%ringSize;
  const s=heap[ringOff+srcIdx];
  if(Math.abs(s)>peak)peak=Math.abs(s);
  scopeRing[scopeRingWP]=s;
  scopeRingR[scopeRingWP]=heap[ringROff+srcIdx];
  scopeSyncRing[scopeRingWP]=heap[syncOff+srcIdx];
  scopeRingWP=(scopeRingWP+1)%4096}
scopeLocalReadPos=writePos;
scopeSamplesAvail=Math.min(scopeSamplesAvail+newSamples,4096);

if(peak<1e-6){scopeHasData=false;return}

// Find two consecutive sync pulses
let endDist=-1,startDist=-1;
for(let s=1;s<=scopeSamplesAvail;s++){
  const idx=(scopeRingWP-s+4096)%4096;
...
if(startDist>0&&endDist>0){
  ...
  if(period>1){
    ...
    const si=(scopeRingWP-startDist+4096)%4096;
    for(let i=0;i<period;i++){scopeDisplay[i]=scopeRing[(si+i)%4096];scopeDisplayR[i]=scopeRingR[(si+i)%4096]}
    scopeHasData=true}}}
...
function drawScopeWaveform(){
  ...
  if(!scopeHasData||scopeDisplayLen<2)return;
...
function drawScopeADSR(){
  ...
  const kAttackTarget=1.2;
  const kReleaseTarget=-0.1;
  const kSustainMs=1000;
  const kThreshold=0.01;

  // J106 mode only in web app
  ...
  const attackMs=1+(Math.ceil(kEnvMax/atkInc)-1)*(1000/kTickRate);

  // Decay time: simulate to threshold
  let decayMs=0;
  ...
  // Release time: simulate to threshold
  let releaseMs=0;
  ...
  // Simulate full ADSR as one continuous curve
  ...
  const curve=new Float32Array(maxTicks+1);
  let envI=0,attacking=true;
  for(let t=0;t<=maxTicks;t++){
    curve[t]=envI/kEnvMax;
  ...
  // Draw (1.5px path, interpolated between ticks)
  ctx.beginPath();
  function envAtPx(px){
    ...
    const frac=tickF-t0;
    const env=curve[t0]+(curve[t0+1]-curve[t0])*frac;
...
function vcfFreqComp(k){return Math.max(1.962/(1+0.197*k),1)}
...
function drawScopeVCF(){
  ...
  const fcSlider=j6?vcfJ6Hz(freqSlider):vcfDacToHz(freqSlider);
  ...
  const fc=fcSlider*vcfFreqComp(k);
...
function buildAboutPixels(w,h){
  ...
  const lines=['ULTRAMASTER','KR-106','2.4.4'];
  const pixels=[];
...
function drawScope(){
  ...
  switch(scopeMode){
    ...
    case 3:drawScopePatchBank();break;
    case 4:drawScopeVCF();break;
